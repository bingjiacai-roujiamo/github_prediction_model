...
# 导入数据
raw_data = pd.read_csv(r"D:\vscode_work\mechine learning\IHC_data.csv")
data = raw_data.copy()
...
categorical_features = ['outcome', 'sex', 'nation', 'infection', 'biopsy',
                        'syringe', 'disease', 'previous_treatment', 'treatment_history',
                        'treatment', 'nuc', 'IFN'] #重新 定义分类变量
continuous_variables = [col for col in data.columns if col not in categorical_features] #重新 定义数值变量

continuous_imputer = IterativeImputer(estimator=RandomForestRegressor(), random_state=RANDOM_STATE, max_iter=10, sample_posterior=False)
...
# 重新读取插补缺失值后的数据
df = pd.read_csv(r'D:/vscode_work/mechine learning/IHC_im.csv')
...
# 确定分类变量
categorical_features = ['sex', 'nation', 'infection', 'biopsy',
                        'syringe', 'disease', 'previous_treatment', 'treatment_history',
                        'treatment', 'nuc', 'IFN']
...
# 贝叶斯超参数优化
mlp_param_bounds = {
    'hidden_layer_sizes_1': (5, 100),
    'hidden_layer_sizes_2': (5, 100),
    'alpha': (0.0001, 0.1),
...
# 加载原始训练数据以获取完整的特征列表
original_df = pd.read_csv('D:/vscode_work/mechine learning/data_im_删e阳无log+比值.csv')
train_columns = original_df.drop('outcome', axis=1).columns.tolist()
...
numeric_features = [col for col in final_selected_features if col not in categorical_features]

# 检查外部验证集中是否缺少任何选定的特征
missing_selected_features = [f for f in final_selected_features if f not in X_external.columns]
if missing_selected_features:
    print(f"警告：外部验证集中缺少以下选定特征: {missing_selected_features}")
    print("无法进行外部验证，因为这些特征是模型所必需的。")
else:
    print("外部验证集包含所有选定的特征。继续进行验证...")
    
    # 为了应用scaler，我们需要处理所有在训练时存在的特征
    # 方法1：创建一个新的scaler，只为存在于外部验证集中的数值特征创建
    print("为外部验证集创建新的scaler...")
    available_numeric_features = [f for f in numeric_features if f in X_external.columns]
    
    # 加载原始训练数据来重新创建scaler
    X_train, y_train, X_test, y_test = joblib.load('D:/vscode_work/mechine learning/train_test_data.pkl')
    
    # 创建新的scaler，只针对外部验证集中存在的特征
    new_scaler = StandardScaler()
    new_scaler.fit(X_train[available_numeric_features])
    
    # 标准化外部验证集
    X_external_scaled = X_external.copy()
    X_external_scaled[available_numeric_features] = new_scaler.transform(X_external[available_numeric_features])
    
    # 检查是否每个选定的特征都在已标准化的外部验证集中
    for feature in final_selected_features:
        if feature not in X_external_scaled.columns:
            print(f"错误：特征 '{feature}' 不在标准化后的外部验证集中。")
            break
    else:
        # 加载模型
        models = {}
